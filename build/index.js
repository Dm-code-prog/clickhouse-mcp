...
server.tool("list_databases", "List all databases on the ClickHouse server", {}, async () => {
    const result = await client.query({
        query: "SHOW DATABASES",
        format: "JSON"
    });
    const databases = await result.json();
    const foundDatabases = `Found the following databases on the ClickHouse server: ${databases.data.map((db) => db.name).join(", ")}`;
    return {
        content: [{
                type: "text",
                text: foundDatabases
...
server.tool("list_tables", "List all tables in a specified database", {
    ...
    try {
        ...
        const foundTables = `Found the following tables in database ${database}: ${tables.data.map((table) => table.name).join(", ")}`;
        return {
            content: [{
                    type: "text",
                    text: foundTables
...
server.tool("run_select_query", "Run a SELECT query on the ClickHouse server. Only SELECT statements are allowed for security reasons.", {
